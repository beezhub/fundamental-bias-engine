...
**Nothing in the description says the work was automated or assisted.** No
"generated by" line, no tool, model or agent name, no robot emoji, no session
link. The rule is in `CONTRIBUTING.md` under Attribution and it covers four
surfaces: commit messages, pull request titles and descriptions, issue bodies
and issue comments. It is written here as well because this is the file a desk
loads before writing a description, and for a long time the rule held in the
tracked files and failed in every pull request body, which is the half a reader
of the repository actually sees.

If the tooling appends such a line on creation, the desk removes it before
leaving the pull request. The rule is about what the description says, not about
who typed it.
