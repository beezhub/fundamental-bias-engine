...
Reopened if a balance history lands, if the owner starts writing entries at
entry, which would let the position limits be performed against today's book,
or if `--force` on an attention limit is used often enough to suggest the cap
is set wrong rather than the trade being wrong.
