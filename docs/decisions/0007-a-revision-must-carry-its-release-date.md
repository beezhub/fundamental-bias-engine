...
The visibility rule, `src/fbe/pillars/base.py:365-371`: an observation carrying
`released_at` is visible when `released_at.date() <= asof`; one without it is
visible when `period + DEFAULT_PUBLICATION_LAG_DAYS[frequency] <= asof`.

The vintage rule, `src/fbe/pillars/base.py:385-387`: where a source republishes a
period, take the highest `revision` among the observations visible at `asof`,
breaking ties on the later `released_at`.
...
`src/fbe/datasources/manual.py:711-712` is the live route in. `released_at` is
optional and `revision` is parsed independently, so a hand-typed correction
...
**`BasePillar._extract` treats an unstamped observation with `revision` greater
