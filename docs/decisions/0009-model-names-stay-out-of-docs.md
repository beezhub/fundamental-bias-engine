...
- `.claude/agents/*.md` carry `model:` in frontmatter, twelve files, eight opus
  and four sonnet. That directory is agent configuration and is not `docs/`.
- `docs/` contains exactly one model token across the whole tree,
  `docs/reasoning-layer.md:90`, naming `claude-opus-5` under a heading describing
  the reasoning layer's own dependency. That is the stated exception.
