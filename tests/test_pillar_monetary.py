...
UNITS = {
    "policy_rate": "percent",
    "yield_2y": "percent",
    "yield_2y_chg_1m": "basis_points",
    "yield_2y_chg_3m": "basis_points",
    "cpi_yoy": "percent",
}

FREQUENCIES = {
    "policy_rate": Frequency.DAILY,
    "yield_2y": Frequency.DAILY,
    "yield_2y_chg_1m": Frequency.DAILY,
    "yield_2y_chg_3m": Frequency.DAILY,
...
def test_without_a_release_stamp_the_assumed_lag_decides(
    ...
    assert lag == 45
    visible = obs("cpi_yoy", "USD", 2.4, date(ASOF.year, ASOF.month, 1))
    visible_period = date.fromordinal(ASOF.toordinal() - lag)
    ready = obs("cpi_yoy", "USD", 2.1, visible_period)

    extracted = pillar._extract([visible, ready], ["USD"], ASOF)

    assert list(extracted["USD"]["cpi_yoy"]) == [ready]


def test_the_assumed_lag_boundary_is_inclusive(pillar: MonetaryPillar) -> None:
    """One day either side of exactly the allowance, so an off-by-one shows."""
    lag = DEFAULT_PUBLICATION_LAG_DAYS[Frequency.MONTHLY]
...
def test_the_five_components_come_back_for_a_complete_currency(
    pillar: MonetaryPillar,
) -> None:
    ...
    assert set(components["USD"]) == set(pillar.component_weights)
...
def test_the_levels_carry_the_percent_unit_they_were_published_in(
    pillar: MonetaryPillar,
) -> None:
    """Read off the `Observation` rather than assumed, so a registry change of
    unit is visible here instead of silently rescaling the pillar."""
    extracted = pillar._extract(full_set("USD"), ["USD"], ASOF)

    assert extracted["USD"]["policy_rate"][-1].unit == "percent"
    assert extracted["USD"]["yield_2y"][-1].unit == "percent"
...
def test_the_changes_carry_the_basis_point_unit(pillar: MonetaryPillar) -> None:
    """A change in percent rather than basis points would be a hundredfold
    error inside a component that is a quarter of the pillar."""
    extracted = pillar._extract(full_set("USD"), ["USD"], ASOF)

    assert extracted["USD"]["yield_2y_chg_1m"][-1].unit == "basis_points"
    assert extracted["USD"]["yield_2y_chg_3m"][-1].unit == "basis_points"
